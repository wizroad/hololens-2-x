...
void VideoFrameProcessor::DumpFrame2UDP(const SoftwareBitmap& softwareBitmap, long long timestamp)
{
    // Compose the output file name
    wchar_t bitmapPath[MAX_PATH];
    swprintf_s(bitmapPath, L"%lld.%s", timestamp, L"bytes");
    ...
    uint32_t imgpacketBufferLength = pixelBufferDataLength / 20;
    uint8_t* imgpacketBufferData = new uint8_t[imgpacketBufferLength];
    for (char i = 0; i < 20; i++)
    {
        imgpacketBufferData[0] = i;
        memcpy(imgpacketBufferData + 1, &pixelBufferData[i * imgpacketBufferLength], sizeof(uint8_t) * imgpacketBufferLength);
        hololens2Sock.SendFrame(imgpacketBufferData, imgpacketBufferLength + 1);
    }
    delete[] imgpacketBufferData;
    /*char temp[256];
    sprintf(temp, "pixelBufferDataLength: %d", pixelBufferDataLength);
    hololens2Sock.SendMsg(temp);*/

    //m_tarball->AddFile(bitmapPath, &pixelBufferData[0], pixelBufferDataLength);    
}

void VideoFrameProcessor::DumpFrame2Http(const SoftwareBitmap& softwareBitmap, long long timestamp)
{
    // Compose the output file name
    wchar_t bitmapPath[MAX_PATH];
    swprintf_s(bitmapPath, L"%lld.%s", timestamp, L"bytes");
    ...
    hololens2Http.SendFrame(pixelBufferData, pixelBufferDataLength);
    hololens2Sock.SendMsg("After HTTp send frame");
    if (hololens2Http.res != CURLE_OK)
    {
        char temp[256];
        sprintf(temp, "curl_easy_perform() failed: %s\n", curl_easy_strerror(hololens2Http.res));
        hololens2Sock.SendMsg(temp);
    }
}
...
void VideoFrameProcessor::CameraWriteThread(VideoFrameProcessor* pProcessor)
{
    while (!pProcessor->m_fExit)
    {
        ...
        if (pProcessor->m_storageFolder != nullptr)
        {
            ...
            {
          //      pProcessor->hololens2Sock.SendMsg("DumpFrame2Http");

                //pProcessor->DumpFrame(softwareBitmap, pProcessor->m_latestTimestamp);
                pProcessor->DumpFrame2Http(softwareBitmap, pProcessor->m_latestTimestamp);
            }
        }
        //pProcessor->hololens2Sock.SendMsg("CameraWriteThread");
    }
}
